...
func IsValid(m commonpb.Money) bool {
	return signMatches(m) && validNanos(m.GetNanos())
}

func signMatches(m commonpb.Money) bool {
...
func IsZero(m commonpb.Money) bool { return m.GetUnits() == 0 && m.GetNanos() == 0 }

// IsPositive returns true if the specified money value is valid and is
// positive.
func IsPositive(m commonpb.Money) bool {
...
// negative.
func IsNegative(m commonpb.Money) bool {
	return IsValid(m) && m.GetUnits() < 0 || (m.GetUnits() == 0 && m.GetNanos() < 0)
}

// AreSameCurrency returns true if values l and r have a currency code and
// they are the same values.
func AreSameCurrency(l, r commonpb.Money) bool {
...
func AreEquals(l, r commonpb.Money) bool {
...
func Negate(m commonpb.Money) commonpb.Money {
...
func Must(v commonpb.Money, err error) commonpb.Money {
	if err != nil {
		panic(err)
	}
	return v
...
// both).
func Sum(l, r commonpb.Money) (commonpb.Money, error) {
	if !IsValid(l) || !IsValid(r) {
		return commonpb.Money{}, ErrInvalidValue
	} else if l.GetCurrencyCode() != r.GetCurrencyCode() {
		return commonpb.Money{}, ErrMismatchingCurrency
	}
...
// to itself n-1 times.
func MultiplySlow(m commonpb.Money, n uint32) commonpb.Money {
	out := m
	for n > 1 {
		out = Must(Sum(out, m))
		n--
