...
func (p *inventory) XaPrepareCreateInventoryProduct(ctx context.Context, req *inventorypb.XaPrepareCreateInventoryProductRequest) (*commonpb.Empty, error) {
	...
	if p.xaPending == nil {
		p.xaPending = map[string]*inventorypb.InventoryProduct{}
	}
	...
	p.xaPending[req.Gid] = &inventorypb.InventoryProduct{Id: req.Id, Stock: req.InitialStock}
	log.Infof("XA: inventory product %s prepared for gid %s", req.Id, req.Gid)
	return &commonpb.Empty{}, nil
}

func (p *inventory) XaCommitCreateInventoryProduct(ctx context.Context, req *commonpb.XaBranchRequest) (*commonpb.Empty, error) {
	p.xaMu.Lock()
	defer p.xaMu.Unlock()
...
func (p *inventory) XaRollbackCreateInventoryProduct(ctx context.Context, req *commonpb.XaBranchRequest) (*commonpb.Empty, error) {
	p.xaMu.Lock()
	defer p.xaMu.Unlock()
